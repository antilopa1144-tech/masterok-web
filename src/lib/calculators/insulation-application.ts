  ...
  switch (application) {
    case INSULATION_APPLICATION.INTERNAL:
      return (
        ...
        "Калькулятор переключён на предварительную каркасную ведомость. Необходимость мембран, стойки, крепёж и весь состав стены определяют по проекту."
      );
    case INSULATION_APPLICATION.ROOF:
      return (
        "Для кровли и мансарды СФТК не используется. " +
        "Калькулятор переключён на предварительную ведомость между стропилами/балками; положение и тип мембран проверяют по полному кровельному пирогу."
      );
    case INSULATION_APPLICATION.FLOOR:
      return (
        "Для пола и перекрытий мокрый фасад и обрешётка «как на стене» не применяются. " +
        "Калькулятор переключён на предварительную ведомость пола; схему по лагам или под стяжку, мембраны и нагрузки он не проектирует."
      );
    case INSULATION_APPLICATION.FOUNDATION:
      return (
        "Для цоколя и фундамента СФТК не применяется. " +
        "Калькулятор оставляет только предварительный расчёт основного материала; крепление, гидроизоляцию, нагрузки и состав подземного узла он не проектирует."
      );
    default:
...
export function applicationMountLabel(
  ...
  if (application === INSULATION_APPLICATION.FLOOR) {
    return "предварительно: между лагами / под стяжку";
  }
  if (application === INSULATION_APPLICATION.INTERNAL) {
    return "предварительная каркасная ведомость";
  }
  if (application === INSULATION_APPLICATION.ROOF) {
    return "предварительно: между стропилами";
  }
  if (application === INSULATION_APPLICATION.FOUNDATION) {
    return "цоколь: основной материал, без проектирования узла";
  }
  if (materialForm === 1) {
