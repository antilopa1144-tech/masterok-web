...
export const puttyDef: CalculatorDefinition = {
  ...
  h1: "Калькулятор шпаклёвки для стен и потолка по площади",
  description:
    "Рассчитайте шпаклёвку для стен и потолка по площади, расходу из техкарты, фактической толщине или числу проходов и фасовке выбранного продукта.",
  metaTitle: withSiteMetaTitle(
    "Калькулятор шпаклёвки для стен по площади",
  ),
  metaDescription:
    "Бесплатный калькулятор шпаклёвки для стен и потолка: рассчитайте расход по площади и техкарте, толщине или проходам, получите целые упаковки.",
  category: "interior",
...
<h2>Пример расчёта шпаклёвки для стен по площади</h2>
<p>Условный пример нужен, чтобы показать механику калькулятора, а не рекомендовать норму. Допустим, площадь стен равна 20 м², в техкарте выбранного продукта указан расход 1,2 кг/м² на 1 мм, средняя толщина — 1,5 мм, а масса упаковки — 20 кг. Тогда чистая потребность: <strong>20 м² × 1,2 кг/м² × 1,5 мм = 36 кг</strong>. Без дополнительной надбавки к покупке нужны <strong>2 упаковки по 20 кг</strong>, всего 40 кг; остаток 4 кг. Для своего объекта перенесите расход и допустимую толщину именно из техкарты выбранной шпаклёвки.</p>
...
<h2>Шпаклёвка стен в квартире в отопительный сезон</h2>
<p>Зимой поверхность рядом с наружной стеной или оконным откосом может отличаться от воздуха в комнате, а работающие радиаторы не гарантируют равномерную сушку слоя. Условия нанесения, допустимую толщину, межслойную выдержку и готовность к шлифованию или финишной отделке проверяйте по техкарте выбранной шпаклёвки. Калькулятор определяет массу и упаковки, но не измеряет температуру основания и влажность.</p>
<p>В цепочке подготовки стен отдельно посчитайте <a href="/kalkulyatory/otdelka/gruntovka/">грунтовку по норме производителя</a>, затем шпаклёвку, а после высыхания основания выберите финиш: <a href="/kalkulyatory/otdelka/kraska/">краску по укрывистости</a> или <a href="/kalkulyatory/otdelka/oboi/">обои по полотнам и рулонам</a>. Если в квартире запотевают окна или остаётся сырость, до финишной отделки проверьте <a href="/kalkulyatory/inzhenernye/ventilyaciya/">воздухообмен и вентиляцию</a>.</p>
...
`,
    faq: [
      {
        question: "Как рассчитать шпаклёвку для стен по площади?",
        answer:
          "<p>Укажите готовую площадь стен или задайте размеры прямоугольной комнаты и вычтите проёмы. Затем перенесите из техкарты выбранного продукта расход и его базу — на 1 мм либо на проход, задайте толщину или число проходов и фактическую фасовку. Пример 1,2 кг/м² при толщине 1,5 мм показывает порядок расчёта, но это не универсальная норма для любой шпаклёвки.</p>",
      },
      {
