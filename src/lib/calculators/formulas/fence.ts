...
Листов профнастила С8 = ⌈Длина_нетто / 1.15⌉

Стандарты:
- Шаг столбов: 2–3 м (оптимально 2.5 м)
- Профнастил С8: рабочая ширина 1.15 м. Для другого профиля сверяйтесь с паспортом производителя.
- В расчёте длины столба заложено 0.9 м подземной части. Это ориентир, а не универсальная глубина: фундамент опор подбирают по грунту, промерзанию, высоте и ветровой нагрузке.
  `,
  howToUse: [
...
<table>
  ...
  <tbody>
    ...
    <tr><td>Заполнение</td><td>C8, 0.45–0.5 мм</td><td>Рулон 10&times;1.5 м</td><td>Штакетина 100 мм</td></tr>
...
<ul>
  ...
  <li><strong>Глубина заделки</strong> — определяется конструкцией опоры, грунтом, промерзанием и ветровой нагрузкой; 0.9 м в результате калькулятора является предварительным ориентиром</li>
...
`,
    faq: [
      ...
      },
      {
        ...
        answer: "<p>По умолчанию расчёт использует профнастил С8 с рабочей шириной 1.15 м:</p><p><strong>N = &lceil;L<sub>нетто</sub> / 1.15&rceil;</strong></p><p>Для забора 50 м (за вычетом ворот 4 м и калитки 1 м): L = 50 &minus; 5 = <strong>45 м</strong></p><p>N = &lceil;45 / 1.15&rceil; = <strong>40 листов</strong></p><p>Для С20, С21 и других профилей рабочая ширина может отличаться — перед покупкой сверьте её с карточкой производителя.</p><p>Высота листа: <strong>2.0 м</strong> (стандарт). Саморезов кровельных: 40 &times; 6 = <strong>240 шт</strong>.</p><p>Поперечные лаги (40&times;20 мм): 45 м &times; 2 ряда = <strong>90 м.п.</strong> (15 штук по 6 м).</p>",
      },
      {
