...
export const tileDef: CalculatorDefinition = {
  ...
  fields: [
    ...
    {
      key: "packArea",
      label: "Площадь плитки в коробке",
      type: "number",
      unit: "м²",
      min: 0.1,
      max: 20,
...
сложность помещения и формат плитки. Итог к покупке округляется вверх до полных коробок по введённой площади упаковки.
Клей и затирка рассчитываются отдельно по нормам расхода.
  `,
  howToUse: [
    "Введите размеры или площадь укладки",
    "Укажите размер плитки, площадь одной коробки и ширину шва",
  ...
  faq: [
    ...
    {
      question: "Почему нужно указывать площадь плитки в коробке?",
      answer:
        "У разных коллекций одного и того же формата в коробке бывает разное количество плиток. Калькулятор использует площадь с упаковки, округляет закупку до целых коробок и отдельно показывает, сколько плиток и какой остаток получится.",
    },
    {
      question: "Нужна ли система выравнивания плитки?",
      answer:
        "Система выравнивания плитки (СВП) полезна для формата примерно от 45 см и крупного керамогранита: она уменьшает перепады по кромкам. Для мозаики и мелкой плитки обычно не нужна. Система не исправляет неровную стяжку — основание готовят заранее.",
    },
  ],
  seoContent: {
    descriptionHtml: `
