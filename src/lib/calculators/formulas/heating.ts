...
export const heatingDef: CalculatorDefinition = {
  ...
  metaTitle: withSiteMetaTitle("Калькулятор радиаторов для квартиры и дома"),
  metaDescription:
  ...
  tags: ["отопление", "радиаторы", "секции радиатора", "радиаторы в квартире", "тепловая нагрузка", "температурный напор"],
  popularity: 75,
  complexity: 3,
  fields: [
    {
...
<h2>Радиаторы в квартире перед отопительным сезоном</h2>
<p>Для квартиры считайте каждую комнату отдельно: одинаковая площадь не означает одинаковые теплопотери у угловой комнаты, помещения над холодным подвалом и комнаты с большой площадью остекления. Если окно продувается или мокнет, сначала проверьте монтажный шов и воздухообмен: для этого есть <a href="/kalkulyatory/otdelka/ustanovka-okon/">калькулятор монтажной пены для окон</a> и <a href="/kalkulyatory/inzhenernye/ventilyaciya/">калькулятор вентиляции</a>.</p>
<p>В многоквартирном доме калькулятор не подтверждает самовольную замену типа прибора, увеличение числа секций или изменение схемы подключения. Доступный температурный режим, гидравлическое сопротивление, байпас, арматуру и допустимую мощность сверяют с проектом дома и требованиями управляющей организации. Если рассматривается электрический обогрев пола как отдельная система, используйте <a href="/kalkulyatory/inzhenernye/teplyy-pol/">калькулятор электрического тёплого пола</a>.</p>
