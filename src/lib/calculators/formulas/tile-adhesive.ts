...
/**
 * Профили формата плитки — источник истины для подписей селекта.
 *
 * Подпись обязана называть те размеры, для которых нормa класса ещё не
 * занижает расход: она идёт верхней границей диапазона. Диапазоны соседних
 * классов не пересекаются, иначе один и тот же размер даёт два разных ответа.
 *
 * `upperCm` — верхняя граница размера в подписи (0 = верхней границы нет),
 * `upperNormCeiling` — верхняя граница базового расхода из опубликованной
 * таблицы приложения (статья «Расход клея для плитки на м2»), с которой
 * сверяется норма класса. Инвариант проверяется тестом.
 */
export const TILE_ADHESIVE_SIZE_CLASSES = [
  { value: 0, label: "до 20×20 см включительно", upperCm: 20, upperNormCeiling: 3.0, modelSideM: 0.3 },
  { value: 1, label: "25×25 — 45×45 см", upperCm: 45, upperNormCeiling: 4.0, modelSideM: 0.45 },
  { value: 2, label: "50×50 — 80×80 см", upperCm: 80, upperNormCeiling: 6.0, modelSideM: 0.6 },
  { value: 3, label: "90×90 см и более (крупноформат)", upperCm: 0, upperNormCeiling: Number.POSITIVE_INFINITY, modelSideM: 0.9 },
] as const;

const LARGE_FORMAT_CLASS = 3;
...
export const tileAdhesiveDef: CalculatorDefinition = {
  ...
  fields: [
    ...
    },
    {
      ...
      type: "select",
      defaultValue: 1,
      options: TILE_ADHESIVE_SIZE_CLASSES.map((item) => ({ value: item.value, label: item.label })),
      hint: "Каждая позиция — это полоса расхода, а не размер плитки: берите ту, в которую попал ваш размер. Четыре укрупнённых профиля модели, а не паспорт конкретного клея; паспортный расход выбранного продукта имеет приоритет. Для крупноформата от 90×90 см модель дополнительно применяет коэффициент 1,70.",
    },
    {
      key: "layingType",
      label: "Место укладки",
    ...
    },
    {
      ...
      type: "switch",
      defaultValue: 0,
      hint: "Ручное включение добавляет фиксированный множитель ×1,70. Для категории от 90×90 см он применяется автоматически, поэтому переключатель скрыт.",
  ...
  calculate(inputs) {
    ...
    const warnings = canonical.warnings.map((warning) => {
      // Предикаты привязаны к началу строки движка: при смене подписей полос
      // их нужно обновлять вместе с engine/tile-adhesive.ts, иначе текст
      // предупреждения уедет в сыром виде (ловилось тестом).
      if (warning.includes("Крупная плитка (50×50")) {
        return "Для полосы 50×50 — 80×80 см модель использует 7,5 кг/м². Зуб шпателя и фактический расход выбирают по наибольшей стороне плитки и техкарте конкретного клея.";
      }
      if (warning.includes("Крупноформат (от 90×90")) {
        return "Для формата от 90×90 см модель автоматически применяет дополнительный множитель ×1,70. Способ нанесения, допустимый формат и полноту контакта проверяют по техкарте выбранного клея и проектным условиям.";
      }
...
- старая плитка: ×1,20;
- дополнительное нанесение: ×1,70 (автоматически для категории от 90×90 см либо вручную);
...
Паспортный расход, зуб шпателя, толщина слоя, класс клея и фактическое основание калькулятор не определяет.
  ...
  howToUse: [
    ...
    "Для категорий до 80×80 см при необходимости вручную включите дополнительное нанесение ×1,70",
...
<table>
  ...
  <tbody>
    <tr><td>до 20&times;20 см включительно</td><td>3 кг/м²</td><td>зуб шпателя, основание, вид плитки</td></tr>
    <tr><td>25&times;25 — 45&times;45 см</td><td>5 кг/м²</td><td>наибольшую сторону и паспортный расход</td></tr>
    <tr><td>50&times;50 — 80&times;80 см</td><td>7,5 кг/м²</td><td>допустимый формат и полноту контакта</td></tr>
    <tr><td>от 90&times;90 см</td><td>7,5 кг/м² до ×1,70</td><td>способ нанесения, класс и деформативность</td></tr>
  </tbody>
</table>
<p>Диапазоны подобраны так, чтобы норма класса не была ниже опубликованной нормы для верхнего размера своей полосы: между 20 и 80 см модель может давать запас относительно табличных значений, и это осознанно — недокупка клея останавливает работу, а лишний мешок нет. Проверяйте по паспорту продукта, если он задаёт меньший расход.</p>
    ...
    faq: [
      {
        ...
        answer: "<p>Для пола, стандартного основания и фасовки 25 кг модель даёт до сценарных множителей: 66 кг для полосы до 20&times;20 см, 110 кг для полосы 25&times;25 — 45&times;45 см и 165 кг для полосы 50&times;50 — 80&times;80 см. Для формата от 90&times;90 см автоматически применяется ×1,70: 280,5 кг. Итоговые карточки MIN/REC/MAX могут отличаться, а покупка округляется вверх до целых мешков.</p><p>Это не таблица конкретного Ceresit, Knauf, Unis или другого продукта. Перед покупкой пересчитайте по паспортной норме выбранного клея.</p>",
      },
      {
