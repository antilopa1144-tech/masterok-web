...
export const balconyDef: CalculatorDefinition = {
  ...
  h1: "Калькулятор площади и обшивки балкона — стены, потолок и панели",
  description:
    "Посчитайте площадь стен и потолка балкона с вычетом окон и двери, затем рассчитайте одну позицию вагонки или панелей до целых упаковок.",
  metaTitle: withSiteMetaTitle(
    "Калькулятор площади и обшивки балкона",
  ),
  metaDescription:
    "Бесплатный калькулятор площади и обшивки балкона: рассчитайте стены и потолок с вычетом проёмов, затем вагонку или панели по размерам и фасовке.",
  category: "interior",
...
<h2>Как посчитать площадь балкона или лоджии</h2>
<p>В режиме простого обмера укажите суммарную длину только тех стен, которые планируете обшить, их высоту и при необходимости размеры потолка. Из полученной площади вычтите остекление, балконную дверь и другие необлицовываемые зоны. Парапет, откосы и короба добавляйте только тогда, когда они действительно закрываются тем же артикулом материала.</p>

<h2>Балкон и лоджия перед холодами</h2>
<p>Сначала определите назначение балкона: холодное помещение с отделкой и отапливаемая лоджия требуют разных решений. Эта страница считает геометрию облицовки и упаковки выбранного материала, но не подтверждает присоединение лоджии к комнате, перенос отопительных приборов или безопасность тёплого контура.</p>
<p>Для монтажного шва остекления используйте <a href="/kalkulyatory/otdelka/ustanovka-okon/">калькулятор установки окон</a>. Количество утеплителя для уже проверенной конструкции посчитает <a href="/kalkulyatory/fasad/uteplenie/">калькулятор утепления дома и квартиры</a>. Если после остекления появляются конденсат и духота, отдельно оцените расход воздуха в <a href="/kalkulyatory/inzhenernye/ventilyaciya/">калькуляторе вентиляции</a>; он не заменяет обследование лоджии и общедомовой вытяжки.</p>
