...
export const primerDef: CalculatorDefinition = {
  ...
  description:
    "Рассчитайте расход грунтовки для стен, потолка или пола по площади, норме производителя, числу слоёв и фактической фасовке.",
  metaTitle: withSiteMetaTitle(
    "Калькулятор грунтовки для стен: расход на 1 м²",
  ),
  metaDescription:
    "Бесплатный калькулятор грунтовки для стен: рассчитайте расход на 1 м² по площади, норме производителя и числу слоёв, получите упаковки и остаток.",
  category: "interior",
  ...
  calculate(inputs) {
    ...
    const warnings = [
      "Калькулятор не подбирает продукт. Совместимость, расход, слои, разбавление и сушку сверьте по техкарте. Литры и килограммы не переводятся без паспортной плотности состава.",
    ];
    ...
    const practicalNotes = [
      ...
      "Разные продукты и нормы считайте отдельными позициями. Валик, кисть, кювета, распылитель и средства защиты подбирайте по способу нанесения и условиям объекта.",
    ];
...
<h2>Грунтовка стен в квартире осенью и зимой</h2>
<p>Работающее отопление и закрытые окна не подтверждают готовность основания к следующему этапу. Перед нанесением и перед продолжением отделки проверьте требования техкарты к температуре основания и воздуха, влажности, вентиляции и времени сушки. Если на наружной стене есть конденсат или устойчивое сырое пятно, расчёт расхода не заменяет поиск причины увлажнения.</p>
<p>Для последовательного ремонта сначала рассчитайте грунтовку, затем отдельной позицией — <a href="/kalkulyatory/otdelka/shpaklevka/">шпаклёвку для стен</a>, а после подготовки основания выберите финиш: <a href="/kalkulyatory/otdelka/kraska/">краску по укрывистости</a> или <a href="/kalkulyatory/otdelka/oboi/">рулоны обоев</a>. При постоянно закрытых окнах и следах сырости проверьте также <a href="/kalkulyatory/inzhenernye/ventilyaciya/">расчёт вентиляции квартиры</a>.</p>

<h2>Что калькулятор не подбирает</h2>
<p>Результат не подтверждает совместимость грунтовки с основанием и следующим покрытием. Валик, кисть, кювета, распылитель и средства защиты также не добавляются автоматически: они зависят от технологии нанесения и объекта. О том, почему срок бывает от нескольких часов до половины суток, читайте в материале <a href="/blog/skolko-sohnet-gruntovka/">«Сколько сохнет грунтовка перед отделкой»</a>. Срок из техкарты можно перенести в <a href="/instrumenty/tajmer-skhvatyvaniya/?preset=primer-deep&amp;from=gruntovka">таймер высыхания грунтовки</a>, но его сигнал остаётся напоминанием о проверке, а не подтверждением готовности основания.</p>
`,
    faq: [
      {
