...
export const concreteDef: CalculatorDefinition = {
  ...
  metaTitle: withSiteMetaTitle("Калькулятор бетона: расчёт материалов онлайн"),
  ...
  fields: [
    ...
    {
      key: "application",
      label: "Применение бетона",
      type: "select",
      defaultValue: 0,
      options: [
        { value: 0, label: "Фундамент / плита на грунте" },
        { value: 1, label: "Стяжка пола" },
        { value: 2, label: "Конструктив (колонна / балка)" },
      ],
      hint: "Влияет на список сопутствующих материалов: для фундамента — гидроизоляция, для конструктива — больше арматуры и опалубки.",
    },
    {
      key: "manualMix",
...
Состав на 1 м³ бетона (цемент М400), те же значения, что в блоке «Пропорции» ниже по странице:
...
**Сопутствующие материалы — расход по применению (по СП 63.13330):**
| Применение         | Арматура, кг/м³ | Опалубка | Гидроизоляция мастикой | Укрывная плёнка |
|--------------------|-----------------|----------|-----------------------|-----------------|
| Фундамент / плита  | 70              | да       | да (боковины)         | да              |
| Стяжка пола        | 25 (сетка)      | нет      | нет                   | да              |
| Конструктив        | 120             | да       | нет                   | да              |

В список покупок не включена вода: она берётся из водопровода. Её объём остаётся в детальном расчёте как справочное значение.
  `,
  howToUse: [
    "Введите требуемый объём бетона в м³",
    "Выберите марку бетона (для фундаментов обычно М200–М250)",
    "Выберите применение: фундамент даст список с обмазочной гидроизоляцией и опалубкой по периметру; стяжка — без мастики и опалубки, с кладочной сеткой вместо прутка; конструктив (колонна, балка) — больше арматуры (~120 кг/м³) и опалубки",
  ...
  faq: [
    {
      ...
      answer: "Марку и класс бетона задают проект и условия участка: нагрузка, тип фундамента, грунт, уровень грунтовых вод, требования к морозостойкости (марка F) и водонепроницаемости (марка W). Для многих частных объектов встречают марки порядка М200–М250, но это не универсальное правило. Калькулятор считает состав для выбранной вами марки; окончательное решение — у проектировщика и по паспорту готовой смеси."
    },
    {
      question: "Можно ли заливать бетон в дождь?",
      answer: "Слабый кратковременный дождь ещё допустим, если смесь не размывается и свежий бетон сразу укрывают от прямой струи воды. Сильный ливень без укрытия размывает верх, портит водоцементное отношение и качество поверхности — заливку лучше перенести или обеспечить тент/плёнку; на плитах и стяжках это особенно критично."
    }
  ],
  seoContent: {
    descriptionHtml: `
