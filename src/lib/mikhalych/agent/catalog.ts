...
export function getCalculatorSchemaPayload(slug: string) {
  ...
  return {
    ...
    hints: [
      ...
      "Для сравнения вариантов («X или Y?») вызови run_calculator несколько раз с теми же размерами, меняя только поле выбора материала/типа (поле с options — напр. insulationType, screedType). Значения вариантов бери из options этого поля.",
    ],
  };
}
