...
describe("площадь комнаты: ответы на поисковые вопросы", () => {
  ...
  it("даёт инструкцию по замеру до ввода размеров", () => {
    const answer = answerFor("Как правильно измерить комнату для расчёта площади?");
    expect(answer).toContain("по линии пола");
    expect(answer).toContain("разделите план на прямоугольники без пересечений");
    expect(answer).toContain("площади отделки");
  });
});
