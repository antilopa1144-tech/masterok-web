...
export const LAMINATE_MODE_OPTIONS: LaminateModeOption[] = [
  { value: "deck-third", label: "Палуба 1/3", desc: "Прямые ряды со смещением стыков на ⅓ доски — сравните подрезки с половинным смещением" },
  { value: "deck-half", label: "Палуба 1/2", desc: "Прямые ряды, смещение на половину доски — выразительный рисунок" },
  { value: "herringbone", label: "Ёлочка", desc: "Доски под 45° парами «рыбья кость» — предварительная оценка закупки по площади" },
];
...
function buildLaminateNotes(
  ...
  if (mode === "deck-third" || mode === "deck-half") {
    notes.push("Минимальное смещение торцевых стыков и длину крайних деталей проверьте в инструкции конкретной коллекции; схема не подтверждает соответствие монтажным требованиям.");
  }
  if (mode === "herringbone") {
    notes.push("Для ёлочки используйте покрытие с подходящей системой соединения по инструкции производителя. Закупка здесь оценивается по площади с запасом, а рисунок не является точной картой раскроя.");
  }
  notes.push("Компенсационные зазоры у стен и препятствий, предельные размеры поля и разрывы в проёмах определяйте по инструкции покрытия; проверьте их отдельно от схемы.");
  return notes;
}
