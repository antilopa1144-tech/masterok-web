...
export default function QuickCalculator({
  ...
  }, [append, calculate, backspace, clear, enableKeyboard, percent]);

  const displayText = display.length > (compact ? 11 : 14)
    ? `${display.slice(0, compact ? 11 : 14)}…`
    : display;
  ...
  const calculator = (
    <div className="card overflow-hidden w-full">
      ...
      <div className={`grid grid-cols-4 ${compact ? "gap-1.5 p-2" : "gap-2 p-3"} bg-slate-50 dark:bg-slate-900`}>
        ...
        {["7", "8", "9"].map((n) => (
          <button key={n} onClick={() => append(n)} className={`${BTN} bg-white dark:bg-slate-800 hover:bg-slate-100 dark:hover:bg-slate-700 text-slate-900 dark:text-slate-100 border border-slate-200 dark:border-slate-700`}>{n}</button>
        ))}
  ...
  if (!showHistory) {
    return <div className={className}>{calculator}</div>;
  }
