...
export function computeCanonicalFacadeInsulation(
  ...
  if (thickness >= 150) {
    warnings.push(
      "Толщина 150 мм и более: допустимость одно- или двухслойной схемы проверьте по проекту и документации выбранной СФТК. Калькулятор считает один слой по площади фасада.",
    );
  }
  if (insulationType === 1) {
    warnings.push(
      "ЭППС применяйте только в предусмотренной проектом зоне и в составе сертифицированной СФТК; подготовку поверхности и противопожарные рассечки калькулятор не определяет.",
    );
  }
  ...
  practicalNotes.push(
    `Количество плит рассчитано для справочной площади ${PLATE_M2} м² с запасом 5%. Толщина меняет описание комплекта, но не число плит на той же площади фасада.`,
  );
  practicalNotes.push(
    insulationType === 0
      ? "Для минваты калькулятор использует 6 тарельчатых дюбелей на 1 м² и запас 5%. Фактическую схему, длину и усиление краевых зон задают проект и документация СФТК."
      : "Для ЭППС калькулятор использует 4 тарельчатых дюбеля на 1 м² и запас 5%. Допустимость материала, схему, длину крепежа и краевые зоны задают проект и документация СФТК.",
  );
  practicalNotes.push(
    "Стартовый профиль оценён по периметру эквивалентного квадрата 4 × √S. Для закупки измерьте фактическую длину цокольной линии фасада.",
  );
  practicalNotes.push(
    "В ведомости одна справочная позиция грунтовки 0,25 л/м² с запасом 10%; калькулятор не разделяет грунты для основания и декоративного слоя.",
  );
