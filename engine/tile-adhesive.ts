...
const TA_DEFAULTS = {
  // tileSize index — полоса расхода, а не категория размера:
  // 0=до 20×20 см включительно, 1=25×25–45×45 см, 2=50×50–80×80 см, 3=от 90×90 см.
  // Границы полос и их подписи синхронизированы со спекой и проверяются тестом:
  // норма класса не должна быть ниже нормы для верхнего размера своей полосы.
  ...
  primer_reserve: 1.15,
  primer_can: 10,
  // tileSize 3 (от 90×90 см) — средняя сторона ~0.9 м (80×80 даёт ~0.8, 90×90 — 0.9).
...
interface TileAdhesiveInputs {
  area?: number;
  /** Класс-полоса формата плитки: 0=до 20×20 см включительно, 1=25×25–45×45 см,
   *  2=50×50–80×80 см, 3=от 90×90 см. Границы полос не пересекаются. */
  tileSize?: number;
  laying?: number;
  base?: number;
  bagWeight?: number;
...
export function computeCanonicalTileAdhesive(
  ...
  if (tileSize === 2) {
    warnings.push("Крупная плитка (50×50 — 80×80 см) — рекомендуется гребёнка 10-12 мм");
  }
  if (tileSize === 3) {
    warnings.push(
      "Крупноформат (от 90×90 см) — по СП 71.13330.2017 обязательно двойное нанесение клея " +
