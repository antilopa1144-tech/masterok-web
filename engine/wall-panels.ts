...
const PANEL_TYPE_LABELS: Record<number, string> = {
  ...
  1: "Древесноволокнистые панели (МДФ, 0,494 м²)",
  2: "3D-панели (0.25 м²)",
...
export function computeCanonicalWallPanels(
  ...
  const mountingGlueNames: Record<number, string> = {
    0: "Монтажный клей без растворителей для пластиковых панелей",
    1: "Монтажный клей для древесноволокнистых панелей",
  ...
  const klaymerNames: Record<number, string> = {
    0: "Монтажные клипсы для пластиковых панелей",
    1: "Кляймеры для древесноволокнистых панелей",
  ...
  const practicalNotes: string[] = [
    `Молдинги и герметик рассчитаны предварительно по периметру эквивалентного квадрата (${roundDisplay(perim, 2).toLocaleString("ru-RU")} м), а не по фактическому контуру стен, углов и проёмов.`,
  ];
  if (mountMethod === 0) {
    practicalNotes.push(
      "Клей принят из расчёта 1 флакон на 4 м², грунтовка — 0,15 л/м²; замените эти нормы расходом из техкарты выбранных составов.",
    );
  } else {
    practicalNotes.push(
      `Обрешётка рассчитана по площади и введённой высоте ${roundDisplay(height, 2).toLocaleString("ru-RU")} м; отдельные стены, углы, проёмы, усиления и вертикальные доборы не моделируются.`,
      "Обрешётку проверяйте уровнем в двух плоскостях; материал подсистемы и крепёж выбирайте по основанию и условиям помещения.",
    );
  }
