...
export function computeCanonicalCeilingInsulation(
  ...
  const practicalNotes: string[] = [
    ...
    "Площади упаковок и размеры плит в расчёте справочные — перед покупкой замените их данными с этикетки выбранного материала.",
  ];
  if (insulationType === 0 || insulationType === 1) {
    practicalNotes.push(
      "Положение и тип пароизоляционного слоя зависят от полного состава перекрытия и температурно-влажностного режима; обеспечьте непрерывность слоя по проекту системы.",
    );
  }
