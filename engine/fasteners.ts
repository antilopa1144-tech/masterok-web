...
const PRACTICAL_NOTES: Record<number, string> = {
  0: "Саморезы для гипсокартона по металлическому профилю — фосфатированные, с мелкой резьбой. Для деревянного каркаса нужен крепёж с крупной резьбой.",
  1: "Для ОСП и фанеры по деревянному каркасу используют саморезы по дереву. Длину и защитное покрытие подбирайте по толщине листа и условиям эксплуатации; для металлического каркаса нужен другой крепёж.",
  2: "Профлист крепят кровельными саморезами с EPDM-шайбой. Длину и схему крепления сверяйте с профилем листа, основанием и ветровой зоной.",
  3: "Номер кляймера подбирают по толщине задней стенки паза. Крепёж самого кляймера выбирают под материал обрешётки.",
};
...
export function computeCanonicalFasteners(
  ...
  const practicalNotes = [PRACTICAL_NOTES[materialType] ?? PRACTICAL_NOTES[0]!];
