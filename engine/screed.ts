...
function buildMaterialsType0(
  ...
  const materials: CanonicalMaterialResult[] = [
    ...
    },
    {
      ...
      subtitle: 'Нужна не для каждой конструкции пола; применять только если разделительный слой предусмотрен проектом или системой',
      quantity: filmArea,
      unit: 'м²',
  ...
  if (meshArea > 0) {
    materials.push({
      ...
      subtitle: 'Добавлена по внутреннему порогу толщины, но необходимость, тип и положение армирования определяют по конструкции пола',
      quantity: meshArea,
      unit: 'м²',
...
export function computeCanonicalScreed(
  ...
  if (thickness > spec.warnings_rules.thick_threshold_mm) {
    warnings.push(
      'Толщина более 100 мм: проверьте нагрузку на перекрытие, состав основания и допустимость одно- или многоэтапного устройства по проекту. Калькулятор не назначает конструкцию слоёв.',
    );
  }
  if (screedType.id === 0 && area > spec.warnings_rules.large_area_cps_threshold_m2) {
    warnings.push(
      'Площадь более 50 м²: заранее согласуйте карты заливки, подачу и производительность замеса. Калькулятор не выбирает между мешками, растворным узлом и механизированной подачей.',
    );
  }

  const practicalNotes: string[] = [];
  practicalNotes.push(
    `Сетка автоматически добавлена при толщине от ${spec.material_rules.mesh_thickness_threshold_mm} мм как справочная позиция. Это не проект армирования: необходимость, материал, ячейку и положение определяют по типу стяжки, основанию и нагрузкам.`,
  );
  practicalNotes.push(
    'Плёнка автоматически показана с запасом 10%, но связанная, разделительная и плавающая стяжки имеют разный состав. Перед покупкой сверьте предусмотренные проектом слои.',
  );
  practicalNotes.push(
    work.inputMode === 0
      ? `Демпферная лента рассчитана по прямоугольному периметру ${roundDisplay(perimeter, 2)} м, без колонн, ниш и внутренних контуров.`
      : 'При вводе только площади периметр принят как 4 × √S. Для демпферной ленты измерьте фактические стены, колонны, ниши и внутренние контуры.',
  );
  practicalNotes.push(
    `Маяки оценены как один профиль на ${BEACONS_AREA_PER_PIECE} м². Фактическое количество зависит от плана помещения, длины правила и принятой технологии.`,
  );
  if (screedType.id === 2) {
    practicalNotes.push(
      'Для полусухой стяжки масса сухих компонентов ориентировочная: рецептуру, фибру, способ подачи и обработки поверхности согласуйте с исполнителем до закупки.',
    );
  }
