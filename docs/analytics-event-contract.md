...
| Событие | Разрешённые параметры | Триггер |
| --- | --- | --- |
| `commerce_materials_open` | `calculator` | Явное раскрытие блока подбора материалов. |
| `commerce_offer_click` | `offer_id`, `calculator` | Явный переход по маркированному предложению. |
| `commerce_cart_open` | `entry_count_bucket` | Открытие корзины с агрегированным числом расчётов. |
| `commerce_checkout_start` | `product` | Явное начало оформления. |
| `commerce_document_download` | `format` | Успешное скачивание PDF или XLSX. |
| `commerce_purchase` | `product`, `value`, `currency` | Только серверное подтверждение покупки; клиент не вызывает его. |
| `commerce_subscription_cancel` | без параметров | Подтверждённая отмена подписки. |

Для commerce-событий запрещены персональные данные, название/состав проекта,
исходные расчётные значения, количества и цены материалов. `value` разрешён
только в `commerce_purchase` и означает подтверждённую денежную сумму в RUB.
