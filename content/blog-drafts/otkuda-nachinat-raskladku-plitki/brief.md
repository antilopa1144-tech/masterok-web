...
- Статус: `published` в Ghost, 5 сентября 2026 года, только на сайте, без email-рассылки.
- Ghost post ID: `6a9bac1ac4aacd28544c731a`.
- Slug: `otkuda-nachinat-raskladku-plitki`.
- Основной URL и canonical: `https://getmasterok.ru/blog/otkuda-nachinat-raskladku-plitki/`.
- Основной продукт: `/instrumenty/raskladka-plitki/`
- Связанный калькулятор: `/kalkulyatory/poly/plitka/`
- Новый slug на основном сайте требует сборки Next.js: `dynamicParams = false`. Публикация в CMS и проверка production — разные этапы.
...
- Одна таблица в числовом примере позволяет сопоставить края и закупку; показ в сниппете не гарантируется.
- Числовой пример связывает информационный запрос с реальным генератором, а не подменяет статью рекламным CTA.
- Уточнения о двери, шве, собственном сдвиге, стене и полу встроены в связный текст без отдельного длинного FAQ.
...
- [MAPEI wall-layout guide](https://www.mapei.com/id/en/blog/detail/blog/2023/01/11/10-tahapan-pasang-keramik-di-dinding): предварительная разметка и проверка раскладки нужны, чтобы не получить тонкие полосы у границ. Проверен 5 сентября 2026 года.
...
Все изображения ранее созданы встроенным генератором изображений OpenAI и сохранены как проектные WebP. Повторно просмотрены 5 сентября 2026 года. В публикации использованы обложка и семь иллюстраций: сетка, замеры, подрезка, ось проёма, пол от входа, остатки и перенос линий. Это поясняющие ИИ-иллюстрации, не фотографии собственного объекта и не точные монтажные чертежи; ограничение явно указано в статье.

### Обложка

- Файл: `images/feature.webp`
- Размер: 1200×630
- Alt: `Два варианта расположения плитки на стене с дверным проёмом`
...
Итого в опубликованной версии: одна обложка, семь внутренних иллюстраций, девять разделов, одна таблица, около 1485 слов по счётчику Ghost. `razmetka-po-tsentru.webp` и `nisha-i-santehnika.webp` не используются; сохранены вместе с исходными промптами ниже как редакционный архив. Актуальные alt и подписи находятся в `article.md` и Ghost.
...
- [x] Заголовок и интент проверены; пользователь разрешил публикацию.
- [x] Числовой пример повторно прогнан на актуальной версии инструмента.
- [x] Ghost Preview на desktop: заголовок и обложка просмотрены; восемь изображений загружаются после исправления адресов.
- [ ] Финальная проверка основного сайта на desktop и mobile после деплоя.
- [x] CTA ведёт именно в генератор раскладки плитки.
- [x] Обратная ссылка добавлена в страницу инструмента; production-проверка после деплоя.
...
## Публикационная проверка 2026-09-05

Ghost Universal Import создал ровно один черновик, но некорректно переписал пути к изображениям. Повторный импорт не выполнялся: семь адресов исправлены в редакторе на `/content/files/images/2026/09/otkuda-nachinat-raskladku-plitki/`, обложка загружена штатной кнопкой в `/content/images/2026/09/feature.webp`. Meta title, description, публичный доступ и canonical проверены в интерфейсе; canonical задан вручную. После этого выполнена публикация «Publish on site».

Числовой пример повторно проверен на текущем `compareTileLayoutStartModes`: от края 45 → 50 плиток, по центру 50 → 55. При условной упаковке по 10 штук это пять и шесть коробок соответственно. Формулы и настройки инструмента не менялись.

Перед публикационным коммитом: lint прошёл; полный запуск тестов дал 3264 успешных и два существующих сбоя SEO-описания sewage в чужих незавершённых изменениях. Они не входят в этот пакет. Проверку CI чистого коммита и фактического production выполнять отдельно.
