...
describe("computeCanonicalConcrete — стандартные входные данные", () => {
  ...
  it("Объём 5 м³, марка M200, ручной замес → цемент, песок, щебень в materials; вода — только в totals", () => {
    ...
    // Вода удалена из списка покупок (берётся из водопровода).
    // Её объём остаётся в totals для технических расчётов.
    expect(names.some((n) => n.includes("Вода"))).toBe(false);
    expect(result.totals.waterL).toBeGreaterThan(0);
  });
